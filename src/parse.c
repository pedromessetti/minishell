...
/*   Updated: 2023/09/06 11:15:52 by annamarianu      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int parse(t_token *tokens, int token_count, char **envp)
 {
    ...
    while (i < token_count) 
    {
        if (tokens[i].type == TOKEN_KEYWORD && strcmp(tokens[i].content, "cd") == 0)
        {
            i++;
        ...
        else if (tokens[i].type == TOKEN_KEYWORD || tokens[i].type == TOKEN_IDENTIFIER)
